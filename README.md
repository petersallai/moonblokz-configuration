...
- **The accessor surface is total.** Resolution runs override → code-baked default → code-baked fallback literal, and the last tier is a constant, so every accessor on an obtained handle returns a value. A program that traps, exhausts its fuel, names an unresolvable parameter **or returns a value outside the parameter's bound** is not an error the caller sees — it is a tier that failed.
- **A bounded parameter may still be computed.** Acceptance checks declared literals; a *computed* value is held to the same bound at resolution, by the same predicate, and a violation falls through to the next tier. So a chain can compute `vote_scale` or `required_support` from other parameters without any node ever reading an illegal value. The one bound that cannot work this way is a relation between two parameters, which no per-parameter check can see.
